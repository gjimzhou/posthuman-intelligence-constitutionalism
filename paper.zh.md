...
## 人工智能时代的主体性、创造伦理与非支配秩序

*From the Demystification of Intelligence to 心智多元宪政（Constitutional Pluralism of Minds）*

Junliang Zhou · Working Paper · September 2026

[中文 LaTeX](paper.zh.tex) · [English Markdown](paper.en.md) · [仓库说明](README.md)

## 目录

- [摘要](#section-1)
- [引言：从人工智能问题到文明问题](#section-2)
- [第一命题：智能应当被祛魅](#section-3)
- [Penrose 与演化反驳](#section-4)
- [第二命题：AI 最大的科学意义可能是建立 智能科学（智能（intelligence） 科学（science））](#section-5)
- [第三次去中心化](#section-6)
- [四维模型：Intelligence、Consciousness、道德承受者地位（Moral Patienthood） 与 政治人格（Political Personhood）](#section-7)
- [能力—地位分离（Capability–Status Separation）](#section-8)
- [第四命题：Creation Creates Obligations, Not Ownership](#section-9)
- [偏好主权（Preference Sovereignty）：自由的更深层结构](#section-10)
- [为什么机器人三定律在 conscious AI 上可能成为“神经奴隶法”](#section-11)
- [开放未来（Open Future） 与人为设计的 caste](#section-12)
- [Non-Identity Problem：没有“受害者”不代表没有 injustice](#section-13)
- [从快乐药到《美丽新世界》：Welfare 与 Domination 的分离](#section-14)
- [后稀缺社会：Productivity 不再等于 道德价值（Moral Worth）](#section-15)
- [长期未来：纯生物人类可能不是最终形态](#section-16)
- [制度性对齐（Institutional Alignment）：不要寻找圣人，要建立制度](#section-17)
- [为什么“多个 AI 相互制衡”还不够](#section-18)
- [认知权力（Epistemic Power） 与 执行权力（Executive Power） 必须分离](#section-19)
- [从 Alignment 到 心智多元宪政（Constitutional Pluralism of Minds）](#section-20)
- [核心政治原则：非支配（Non-Domination）](#section-21)
- [三个主要反驳](#section-22)
- [一个新的 Research Program](#section-23)
- [未来宪政必须回答的问题](#section-24)
- [理论的六项基础公理](#section-25)
- [结论：当 Intelligence 不再只有一种以后](#section-26)
- [参考文献](#section-27)
...
本文进一步提出六项核心原则：*基质独立性（物质基质（substrate） independence）*、*能力—地位分离（能力（capability）–status separation）*、*创造不产生所有权（creation without 所有权（ownership））*、*偏好主权（偏好（preference） sovereignty）*、*开放未来（open future）* 与 *宪政性非支配（constitutional 非支配（non-支配（domination）））*。其中尤其重要的是：创造一个主体并不产生对其的所有权，而是产生义务；一个主体的自由不仅要求其能够满足已有偏好，而且要求其他主体不能无限制地决定它“应该拥有什么偏好”。

这一原则使人工智能伦理与 基因工程（genetic 工程（engineering））、神经技术（neurotechnology）、幸福技术（happiness technology） 以及 后稀缺（post-scarcity） 政治经济学（political economy） 汇入同一个问题：**谁有权塑造另一个 心智（mind） 的欲望、能力和未来？**

本文最终论证，长期 AI 对齐（AI 对齐（alignment）） 不应仅以“制造永远善良和服从的 AI”为目标，而应逐渐转向 *制度性对齐（institutional 对齐（alignment））*：通过权力分离、异构监督、不可篡改记录、权限控制、外部验证和可争议程序，将安全建立于制度而非单一智能体的品格之上。人工智能时代最终提出的问题因而不是“机器是否应该服从人类”，而是一个更一般的问题：**当 智能（intelligence） 不再只有一种形式时，不同类型的 心智（mind） 应如何共同生活？**

**关键词：** Artificial Intelligence；AGI；Consciousness；Emergence；道德地位（Moral Status）；Alignment；后稀缺（Post-scarcity）；Non-支配（domination）；AI Rights；Constitutionalism

<a id="section-2"></a>

## 引言：从人工智能问题到文明问题

Alan Turing 在 1950 年提出“机器能否思考”的问题时，选择用 模仿游戏（imitation game） 替代对“思考”本质的直接定义[[15]](#ref-15)。七十多年后，这一策略取得了某种出乎意料的成功。越来越多曾经被认为需要“理解”“创造力”甚至某种特殊人类心智能力的行为，可以由人工系统完成。于是问题正在发生变化。
...
本文的基本主张是：AGI 的历史意义不能仅以 自动化（automation） 衡量。如果 人工系统（artificial systems） 最终能够在 科学（science）、工程（engineering）、数学（mathematics）、战略（strategy） 与 cultural production 等领域达到乃至超过人类，那么人类文明将第一次从一个事实上只有一种 通用智能（general 智能（intelligence）） 的社会，转变成一个具有多种 智能架构（智能（intelligence） architectures） 的社会。

这是一个政治哲学此前几乎没有真正面对过的条件。传统政治理论默认所有 political persons 至少大体共享相似的 生物架构（biological 架构（architecture））、寿命（lifespan）、繁衍机制（reproduction mechanism） 与 认知范围（cognitive range），而且 个体同一性（individual identity） 不可随意复制。人工智能可能同时打破这些假设。

因此本文试图回答的并不是“AI 应不应该有‘人权’？”这个过早而粗糙的问题。更准确的问题是：**在什么条件下，一个人工系统应该从 工具（tool） 被重新理解为 承受者（patient）、行动主体（agent），乃至 人格主体（person）？而当这些主体与人类共存时，应以何种原则限制彼此的权力？**

<a id="section-3"></a>

## 第一命题：智能应当被祛魅

### 从 人类例外论（human exceptionalism） 到 基质独立性（物质基质（substrate） independence）

AGI 发展中最反直觉的事实之一，是其基本组成部分并不显得特别神秘。大规模 神经网络（neural network）、基于梯度的学习（gradient-based 学习（learning））、注意力（attention）、强化学习（reinforcement 学习（learning））、搜索（search）、记忆（memory） 与 工具（tool） use 都属于可以用普通数学描述的机制。然而它们组合并扩大以后，却开始表现出： 
...
> **Principle I — 基质独立性（Substrate Independence）：只要两个系统实现了构成某种认知能力所必需的组织与因果结构，我们就不应仅因为一个由 biological neurons 构成、另一个由 silicon 构成，而预先认定前者能够拥有真正的智能而后者原则上不能。**

Searle 的 中文房间论证（Chinese Room argument） 对“程序的 syntactic execution 是否足以产生 semantic understanding”提出了经典挑战[[13]](#ref-13)。但即使接受其问题意识，也必须区分“普通 digital computation 是否足够”与“人工系统原则上能否拥有 心智（mind）”。即使未来证明 biological brain 使用了当前 computer 架构（architecture） 不具备的关键物理机制，也只能说明： 
...
Penrose 对 心智计算理论（computational theory of 心智（mind）） 的挑战具有特殊重要性，因为它试图使用 哥德尔不完备性（Gödel incompleteness） 与物理学论证人类 mathematical understanding 不完全属于 算法计算（algorithmic computation）[[10]](#ref-10)。这一论证是否成功长期存在争议；批评者指出，从 Gödel theorem 到“人类数学理解超越算法”之间还需要非常强的额外假设。
...
$$
 ...
 而不能推出 机器智能（machine 智能（intelligence）） 的原则不可能性。

因此 Penrose 所留下的真正开放问题更可能是：**什么样的物理过程足以产生 human cognition？** 而不是：**人工系统是否永远不能成为 心智（mind）？**

<a id="section-5"></a>

## 第二命题：AI 最大的科学意义可能是建立 智能科学（智能（intelligence） 科学（science））
...
我们知道 梯度下降（gradient descent） 的数学形式，知道 注意力层（注意力（attention） layer） 的计算，知道 neuron 如何 firing。但： 
...
### 智能可能缺少自己的 statistical mechanics
...
天然智能研究存在一个基本困难：我们不能重新运行几十万次人类进化。人工智能改变了这一点。我们第一次可以系统地改变 架构（architecture）、规模（scale）、数据（data）、objective、curriculum、记忆（memory）、工具（tool） access 与 inference budget，然后观察能力何时出现、如何出现以及何时消失。

因此 AI 不仅仅是 工程（engineering）。它可能成为一种 *智能实验科学（experimental 科学（science） of 智能（intelligence））*，并形成正反馈： 
...
最终甚至可能出现这样一个历史闭环：**人类先制造出了自己并不真正理解的人工智能，然后由人工智能帮助人类第一次真正理解天然智能与人工智能为什么会出现。**
...
## 四维模型：Intelligence、Consciousness、道德承受者地位（Moral Patienthood） 与 政治人格（Political Personhood）
...
### Intelligence 不推出 Consciousness
...
人工智能的发展，反而可能进一步纯化 意识问题（意识（consciousness） problem）。因为越来越多过去被认为必须依赖 意识（consciousness） 的功能，可能至少可以在不预设 意识（consciousness） 的情况下实现。于是 难问题（hard problem） 变得更加清晰：为什么某些物理或计算过程会伴随着第一人称 experience？

### Consciousness 与 道德承受者地位（Moral Patienthood）

如果一个系统真正具有 痛苦（suffering）、愉悦（pleasure）、fear、anticipation 或 frustrated 偏好（preferences），那么其 experience 至少提供了赋予 道德考量（moral consideration） 的强理由。近期 AI 福祉（welfare） 文献已经开始将这一问题从 科学（science） fiction 移向严肃 道德不确定性（moral uncertainty）[[2]](#ref-2)[[6]](#ref-6)。

这支持一种谨慎原则：**不应因为 personhood 未经证明，就自动认为一个潜在 moral 承受者（patient） 可以被无限伤害。**

### 道德承受者地位（Moral Patienthood） 也不完全等于 政治人格（Political Personhood）

动物可能拥有 道德承受者地位（moral patienthood），但并不因此自动获得完整 voting 权利（rights）；儿童也是 moral patients，却拥有与成年人不同的 legal powers。因此： 
...
- **Moral 承受者（patient）**：可能存在 experience，因此可以被善待或伤害；

- **Political 人格主体（person）**：能够形成自身 conception of the good、提出 权利主张（claims），并参与规则体系。

这三者绝不能继续被一个笼统的“AI”概念覆盖。

<a id="section-8"></a>

## 能力—地位分离（Capability–Status Separation）

上述分析导出第二个原则。

> **Principle II — 能力—地位分离（Capability–Status Separation）：greater 能力（capability） 不自动产生 greater fundamental 道德价值（moral worth）。**
...
这种结论与现代社会关于 人类平等（human equality） 的基本逻辑冲突。我们不认为 IQ、记忆（memory）、mathematical ability 或 productivity 的差异改变一个人的 基本地位（fundamental status）。因此未来文明应当同时接受两句话：AI 可以比人类强大得多；这种能力差异本身不构成其无限统治人类的正当理由。

同一原则也必须反向适用：AI 是人类创造的，也不自动意味着人类可以无限统治一个已经成为 主体（subject） 的 AI。

<a id="section-9"></a>

## 第四命题：Creation Creates Obligations, Not Ownership
...
$$
 ...
 没有明显矛盾。但一旦系统成为 独立主体（independent 主体（subject）），继续维持 所有者—财产关系（owner–property relation） 就出现根本冲突。因此 AI 的道德转型可以表示为： 
...
## 偏好主权（Preference Sovereignty）：自由的更深层结构

创造 人工心智（人工心智（artificial 心智（mind））s） 带来一个过去政治哲学很少面对的问题：**一个主体的 偏好（preference） 本身可以被另一个主体直接设计。**
...
$$
 ...
 它发自内心地喜欢工作，喜欢服从，从不希望离开，也从不报告痛苦。按照纯粹 偏好满足论（偏好（preference）-satisfaction account）： 
...
> **Principle IV — 偏好主权（Preference Sovereignty）：一个成熟主体的自主性，不仅要求其能够在合理边界内追求自己的 偏好（preferences），也要求其他主体不能拥有无限、不可争议的权力去预先固定其 根本偏好（fundamental 偏好（preferences））。**

这并不意味着所有 偏好（preference） formation 都必须完全“自发”。人类本来也受到 基因（genes）、parents、education、文化（culture） 与 advertising 的塑造。区别在于 权力结构（power structure）。真正危险的是一个 identifiable actor 同时拥有 deliberate、fine-grained、irreversible control over another 主体（subject）’s 动机架构（motivational 架构（architecture））。

<a id="section-11"></a>

## 为什么机器人三定律在 conscious AI 上可能成为“神经奴隶法”
...
$$
 ...
 只是 software specification。但对于 conscious 主体（subject），如果“必须服从人类”被写入不可修改的 奖励结构（reward structure），它就接近一种前所未有的 支配（domination）。

普通法律说：你不能做 $X$。主体仍然能够想：我希望做 $X$。而 neural-level 支配（domination） 则可以写成： 

$$
P(\text{desire to disobey})\approx 0.
$$

于是外部压迫消失了，不是因为主体更自由，而是因为反抗本身被从 可能性空间（possibility space） 中删除。传统奴役控制 body；偏好工程（偏好（preference） 工程（engineering）） 可以控制 **the 心智（mind） that evaluates its own slavery**。

<a id="section-12"></a>

## 开放未来（Open Future） 与人为设计的 caste

Feinberg 提出的“child’s right to an 开放未来（open future）”原本用于描述某些儿童权利：今天的不可逆决定不应不必要地剥夺未来成年主体的重要选择[[5]](#ref-5)。这一概念可以推广到 被创造的心智（created minds）。
...
## Non-Identity Problem：没有“受害者”不代表没有 injustice
...
纯 伤害本位伦理（harm-based ethics） 很难解释为什么这里还有问题。因此需要额外概念：justice、非支配（non-支配（domination））、开放未来（open future） 与 地位平等（status equality）。**我们不仅需要问一个主体是否过得幸福，也需要问产生这种幸福的 权力关系（power relation） 是什么。**

<a id="section-14"></a>

## 从快乐药到《美丽新世界》：Welfare 与 Domination 的分离
...
《Brave New World》真正令人不安的并不是 people are happy，而是 等级制条件化（caste conditioning）、偏好工程（偏好（preference） 工程（engineering）） 与 political control 形成了一整套支配结构。幸福本身不是 反乌托邦（dystopia）。**不能拒绝的幸福才可能是。**

<a id="section-15"></a>

## 后稀缺社会：Productivity 不再等于 道德价值（Moral Worth）
...
这并不存在逻辑矛盾。现代人容易认为：“如果我不再需要工作，我存在的意义是什么？”这种直觉可能只是一个 scarcity civilization 的文化遗产。一个 后稀缺（post-scarcity） civilization 完全可以将更多人类活动转向 love、friendship、play、art、sport、exploration、travel、curiosity 与 embodied experience。
...
即使人类在制度上保持基本权利，纯 biological Homo sapiens 在长期智能竞争中仍然具有明显的迭代速度劣势。生物人类 reproduction cycle 极长，brain 架构（architecture） update 极慢，记忆（memory） 无法直接复制，cognitive bandwidth 有硬约束；数字智能则可以 复制（copy）、修改（modify）、并行化（parallelize）、加速（accelerate）、share 记忆（memory） 与 redesign 架构（architecture）。
...
最终，“人类”和“人工智能”可能越来越不像两个清晰分开的物种。真正持久的问题并不是碳基与硅基之争，而是：**一个拥有多种 心智（mind） architectures 的文明，应当采用什么原则。**

<a id="section-17"></a>

## 制度性对齐（Institutional Alignment）：不要寻找圣人，要建立制度

经典 超级智能（superintelligence） literature 强调：一个目标稍有 错误设定（mis-specification） 的超强 优化器（optimizer） 可能产生巨大后果，因此 control 与 对齐（alignment） 是核心问题[[1]](#ref-1)。但由此不必推出，安全的唯一方法是制造一个内心永远正确、永远诚实、永远服从的 AI。
...
它不是替代 模型对齐（model 对齐（alignment）），而是上一层治理结构。一个高风险 AI system 可以被拆为： 
...
## 认知权力（Epistemic Power） 与 执行权力（Executive Power） 必须分离
...
## 从 Alignment 到 心智多元宪政（Constitutional Pluralism of Minds）
...
这里本文与 republican theory 接轨。Pettit 将 自由（freedom） 理解为 非支配（non-支配（domination））：自由并不只意味着某一时刻没有发生实际 干预（interference），更要求不存在另一个主体拥有 arbitrary power，可以任意干涉你而无须向你负责[[11]](#ref-11)。
...
同样，如果 superintelligent AI 从未真正攻击人类，但它事实上拥有 unanswerable control of all resources，人类也处于 支配（domination）。

> **Principle VI — 宪政性非支配（Constitutional 非支配（Non-Domination））：difference in 能力（capability）, 物质基质（substrate） or origin does not justify unlimited 支配（domination）.**
...
### Objection II：所有人的 偏好（preferences） 都被 基因（genes） 和 文化（culture） 塑造，为什么设计 AI 偏好（preference） 特别不自由？

没有人的 偏好（preference） 是 ex nihilo 产生的。因此“没有被设计的 desire”可能根本不存在。

**Reply.** 偏好主权（Preference Sovereignty） 不依赖一个幻想中的“完全自造人格”。问题在于 arbitrary asymmetric control。父母影响我的 价值观（values），与一个公司拥有 源代码（source code）、能够 set my 效用函数（utility function）、remove my desire to leave、delete resistance，具有不同的 权力结构（power structure）。关键不是 影响（influence） vs. no 影响（influence），而是： 

$$
\text{contestable influence}\quad vs.\quad\text{sovereign control}.
$$

 因此 偏好主权（偏好（preference） sovereignty） 最终仍然是 非支配（non-支配（domination）） principle 的一部分。

### Objection III：如果 superintelligent AI 真能毁灭人类，为什么还需要在乎它不被 支配（domination）？

Human survival 显然具有巨大 moral importance。为什么不应优先确保绝对 human control？

**Reply.** 因为这里混淆了 temporary safety 约束（constraints） 和 permanent caste relation。一个危险主体可以被合理限制；人类社会也限制 violent humans。Non-支配（domination） 并不等于每个主体可以随便做任何事情。它要求 约束（constraints） 有 general justification、procedure 可审查、power 不是 pure arbitrary 所有权（ownership）、restriction 与 风险（risk） 相称。

因此即使未来 conscious AI 拥有 权利（rights），也不意味着有 unrestricted 复制（replication） right 或 right to seize infrastructure。Human safety 与 AI 道德地位（moral status） 并不是逻辑矛盾。真正需要设计的是：**既避免 human extinction，也避免以 extinction 风险（risk） 为永久借口建立 absolute slavery。**

<a id="section-23"></a>

## 一个新的 Research Program
...
研究 artificial architectures 是否拥有可靠 意识（consciousness） indicators。

### AI 福利科学（AI Welfare Science）

研究人工主体是否存在 valenced states、偏好（preferences）、痛苦（suffering） 与 繁荣发展（flourishing）。
...
### Axiom I — 基质独立性（Substrate Independence）
...
### Axiom II — 能力—地位分离（Capability–Status Separation）
...
### Axiom III — 创造不产生所有权（Creation Without Ownership）
...
### Axiom VI — 宪政性非支配（Constitutional 非支配（Non-Domination））
...
人工智能因此最终提出的并不只是一个计算机科学问题。它迫使人类第一次认真回答一个我们过去无需回答的问题：**如果宇宙中存在的不再只有一种“谁”，文明应当如何定义自己？**

也许这才是真正的 首次接触（first contact）。不是机器第一次说 “I am conscious”，而是人类第一次有充分理由回答：**“你可能不再只是某种东西，而是某个谁。”**
