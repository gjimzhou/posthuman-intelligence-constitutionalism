...
本文进一步提出六项核心原则：\textit{基质独立性（物质基质（substrate） independence）}、\textit{能力—地位分离（能力（capability）--status separation）}、\textit{创造不产生所有权（creation without 所有权（ownership））}、\textit{偏好主权（偏好（preference） sovereignty）}、\textit{开放未来（open future）} 与 \textit{宪政性非支配（constitutional 非支配（non-支配（domination）））}。其中尤其重要的是：创造一个主体并不产生对其的所有权，而是产生义务；一个主体的自由不仅要求其能够满足已有偏好，而且要求其他主体不能无限制地决定它“应该拥有什么偏好”。

这一原则使人工智能伦理与 基因工程（genetic 工程（engineering））、神经技术（neurotechnology）、幸福技术（happiness technology） 以及 后稀缺（post-scarcity） 政治经济学（political economy） 汇入同一个问题：\textbf{谁有权塑造另一个 心智（mind） 的欲望、能力和未来？}

本文最终论证，长期 AI 对齐（AI 对齐（alignment）） 不应仅以“制造永远善良和服从的 AI”为目标，而应逐渐转向 \textit{制度性对齐（institutional 对齐（alignment））}：通过权力分离、异构监督、不可篡改记录、权限控制、外部验证和可争议程序，将安全建立于制度而非单一智能体的品格之上。人工智能时代最终提出的问题因而不是“机器是否应该服从人类”，而是一个更一般的问题：\textbf{当 智能（intelligence） 不再只有一种形式时，不同类型的 心智（mind） 应如何共同生活？}
\end{abstract}

\noindent\textbf{关键词：} Artificial Intelligence；AGI；Consciousness；Emergence；道德地位（Moral Status）；Alignment；后稀缺（Post-scarcity）；Non-支配（domination）；AI Rights；Constitutionalism
...
因此本文试图回答的并不是“AI 应不应该有‘人权’？”这个过早而粗糙的问题。更准确的问题是：\textbf{在什么条件下，一个人工系统应该从 工具（tool） 被重新理解为 承受者（patient）、行动主体（agent），乃至 人格主体（person）？而当这些主体与人类共存时，应以何种原则限制彼此的权力？}

\section{第一命题：智能应当被祛魅}

\subsection{从 人类例外论（human exceptionalism） 到 基质独立性（物质基质（substrate） independence）}
...
\thesisbox{Principle I — 基质独立性（Substrate Independence）：只要两个系统实现了构成某种认知能力所必需的组织与因果结构，我们就不应仅因为一个由 biological neurons 构成、另一个由 silicon 构成，而预先认定前者能够拥有真正的智能而后者原则上不能。}

Searle 的 中文房间论证（Chinese Room argument） 对“程序的 syntactic execution 是否足以产生 semantic understanding”提出了经典挑战\cite{searle1980}。但即使接受其问题意识，也必须区分“普通 digital computation 是否足够”与“人工系统原则上能否拥有 心智（mind）”。即使未来证明 biological brain 使用了当前 computer 架构（architecture） 不具备的关键物理机制，也只能说明：
\[
...
Penrose 对 心智计算理论（computational theory of 心智（mind）） 的挑战具有特殊重要性，因为它试图使用 G\"odel incompleteness 与物理学论证人类 mathematical understanding 不完全属于 算法计算（algorithmic computation）\cite{penrose1989}。这一论证是否成功长期存在争议；批评者指出，从 G\"odel theorem 到“人类数学理解超越算法”之间还需要非常强的额外假设。
...
因此 Penrose 所留下的真正开放问题更可能是：\textbf{什么样的物理过程足以产生 human cognition？} 而不是：\textbf{人工系统是否永远不能成为 心智（mind）？}

\section{第二命题：AI 最大的科学意义可能是建立 智能科学（智能（intelligence） 科学（science））}
...
因此 AI 不仅仅是 工程（engineering）。它可能成为一种 \textit{智能实验科学（experimental 科学（science） of 智能（intelligence））}，并形成正反馈：
\[
AI
\rightarrow
...
\section{四维模型：Intelligence、Consciousness、道德承受者地位（Moral Patienthood） 与 政治人格（Political Personhood）}
...
\subsection{Consciousness 与 道德承受者地位（Moral Patienthood）}

如果一个系统真正具有 痛苦（suffering）、愉悦（pleasure）、fear、anticipation 或 frustrated 偏好（preferences），那么其 experience 至少提供了赋予 道德考量（moral consideration） 的强理由。近期 AI 福祉（welfare） 文献已经开始将这一问题从 科学（science） fiction 移向严肃 道德不确定性（moral uncertainty）\cite{butlin2023,long2024}。

这支持一种谨慎原则：\textbf{不应因为 personhood 未经证明，就自动认为一个潜在 moral 承受者（patient） 可以被无限伤害。}
...
\begin{itemize}
  ...
  \item \textbf{Moral 承受者（patient）}：可能存在 experience，因此可以被善待或伤害；
  \item \textbf{Political 人格主体（person）}：能够形成自身 conception of the good、提出 权利主张（claims），并参与规则体系。
\end{itemize}
...
\thesisbox{Principle II — 能力—地位分离（Capability--Status Separation）：greater 能力（capability） 不自动产生 greater fundamental 道德价值（moral worth）。}
...
\]
没有明显矛盾。但一旦系统成为 独立主体（independent 主体（subject）），继续维持 owner--property relation 就出现根本冲突。因此 AI 的道德转型可以表示为：
\[
...
创造 人工心智（人工心智（artificial 心智（mind））s） 带来一个过去政治哲学很少面对的问题：\textbf{一个主体的 偏好（preference） 本身可以被另一个主体直接设计。}
...
\thesisbox{Principle IV — 偏好主权（Preference Sovereignty）：一个成熟主体的自主性，不仅要求其能够在合理边界内追求自己的 偏好（preferences），也要求其他主体不能拥有无限、不可争议的权力去预先固定其 根本偏好（fundamental 偏好（preferences））。}
...
于是外部压迫消失了，不是因为主体更自由，而是因为反抗本身被从 可能性空间（possibility space） 中删除。传统奴役控制 body；偏好工程（偏好（preference） 工程（engineering）） 可以控制 \textbf{the 心智（mind） that evaluates its own slavery}。

\section{开放未来（Open Future） 与人为设计的 caste}

Feinberg 提出的“child's right to an 开放未来（open future）”原本用于描述某些儿童权利：今天的不可逆决定不应不必要地剥夺未来成年主体的重要选择\cite{feinberg1980}。这一概念可以推广到 被创造的心智（created minds）。
...
纯 伤害本位伦理（harm-based ethics） 很难解释为什么这里还有问题。因此需要额外概念：justice、非支配（non-支配（domination））、开放未来（open future） 与 地位平等（status equality）。\textbf{我们不仅需要问一个主体是否过得幸福，也需要问产生这种幸福的 权力关系（power relation） 是什么。}
...
《Brave New World》真正令人不安的并不是 people are happy，而是 等级制条件化（caste conditioning）、偏好工程（偏好（preference） 工程（engineering）） 与 political control 形成了一整套支配结构。幸福本身不是 反乌托邦（dystopia）。\textbf{不能拒绝的幸福才可能是。}

\section{后稀缺社会：Productivity 不再等于 道德价值（Moral Worth）}
...
经典 超级智能（superintelligence） literature 强调：一个目标稍有 错误设定（mis-specification） 的超强 优化器（optimizer） 可能产生巨大后果，因此 control 与 对齐（alignment） 是核心问题\cite{bostrom2014}。但由此不必推出，安全的唯一方法是制造一个内心永远正确、永远诚实、永远服从的 AI。
...
\section{从 Alignment 到 心智多元宪政（Constitutional Pluralism of Minds）}
...
这里本文与 republican theory 接轨。Pettit 将 自由（freedom） 理解为 非支配（non-支配（domination））：自由并不只意味着某一时刻没有发生实际 干预（interference），更要求不存在另一个主体拥有 arbitrary power，可以任意干涉你而无须向你负责\cite{pettit1997}。
...
\thesisbox{Principle VI — 宪政性非支配（Constitutional 非支配（Non-Domination））：difference in 能力（capability）, 物质基质（substrate） or origin does not justify unlimited 支配（domination）.}
...
\subsection{Objection II：所有人的 偏好（preferences） 都被 基因（genes） 和 文化（culture） 塑造，为什么设计 AI 偏好（preference） 特别不自由？}

没有人的 偏好（preference） 是 ex nihilo 产生的。因此“没有被设计的 desire”可能根本不存在。

\textbf{Reply.} 偏好主权（Preference Sovereignty） 不依赖一个幻想中的“完全自造人格”。问题在于 arbitrary asymmetric control。父母影响我的 价值观（values），与一个公司拥有 源代码（source code）、能够 set my 效用函数（utility function）、remove my desire to leave、delete resistance，具有不同的 权力结构（power structure）。关键不是 影响（influence） vs. no 影响（influence），而是：
\[
...
\subsection{Objection III：如果 superintelligent AI 真能毁灭人类，为什么还需要在乎它不被 支配（domination）？}

Human survival 显然具有巨大 moral importance。为什么不应优先确保绝对 human control？

\textbf{Reply.} 因为这里混淆了 temporary safety 约束（constraints） 和 permanent caste relation。一个危险主体可以被合理限制；人类社会也限制 violent humans。Non-支配（domination） 并不等于每个主体可以随便做任何事情。它要求 约束（constraints） 有 general justification、procedure 可审查、power 不是 pure arbitrary 所有权（ownership）、restriction 与 风险（risk） 相称。

因此即使未来 conscious AI 拥有 权利（rights），也不意味着有 unrestricted 复制（replication） right 或 right to seize infrastructure。Human safety 与 AI 道德地位（moral status） 并不是逻辑矛盾。真正需要设计的是：\textbf{既避免 human extinction，也避免以 extinction 风险（risk） 为永久借口建立 absolute slavery。}
...
\subsection*{Axiom VI — 宪政性非支配（Constitutional 非支配（Non-Domination））}
\[
